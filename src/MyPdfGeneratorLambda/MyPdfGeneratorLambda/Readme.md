...
WORKDIR /src
COPY ["MyPdfGeneratorLambda.csproj", "MyPdfGeneratorLambda/"]
RUN dotnet restore "MyPdfGeneratorLambda/MyPdfGeneratorLambda.csproj"

WORKDIR "/src/MyPdfGeneratorLambda"
COPY . .
RUN dotnet build "MyPdfGeneratorLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MyPdfGeneratorLambda.csproj" \
...
Execute unit tests
```
cd "MyPdfGeneratorLambda/test/MyPdfGeneratorLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "MyPdfGeneratorLambda/src/MyPdfGeneratorLambda"
